...
[ExecuteInEditMode]
public class MapBuilderManagerForPickupMechanics : MonoBehaviour
{
	...
	void OnEnable()
    {
		...
		containerInstances = new GameObject[levels.Length][,];
    ...
    public void MakeLevel()
    {
		...
		currentMapIndex = Mathf.Clamp( currentMapIndex, 0, Mathf.Max( 0, levels.Length - 1 ) );
...
////////////////////////////////////////// ITEMS PLACER ////////////////////////////
	...
	public void PlaceItems(){
		...
		Map currentMap = levels[currentMapIndex];
		...
		for (int i=0; i<currentMap.itemTypes.Length; i++){
			...
			arrayHolderComponent.UpdateCoordinatesInOccupancyMap( globalCoordinates, PickUpMechanics.occupied );
	...
	void PlaceContainers(){
		var currentMap = levels[currentMapIndex];
		...
		for( int i=0; i< currentMap.mapSizeX; i++){
			for (int j=0; j< currentMap.mapSizeY; j++){
				...
				containerComponent.coordinates = new Vector2( i, j );
...
/////////////////////// DEBUGS /////////////////////////////////////////////////////
	...
	void OnDrawGizmos()
	{
		...
		if( levels == null ){
			...
			currentMapIndex = Mathf.Clamp(currentMapIndex, 0, levels.Length-1 );
			var currentMap = levels[currentMapIndex];
			...
			itemsX = (int)levels[currentMapIndex].mapSizeX;
			itemsY = (int)levels[currentMapIndex].mapSizeY;
		}
